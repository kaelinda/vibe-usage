...
AI Token 使用量监控器 - 实时监控跨平台的 AI API token 使用情况。
...
- **模型类型分类**：自动识别模型类型（通用/视觉/推理），并在仪表盘显示
...
#### 模型类型检测
- **通用（General）**：标准语言模型，如 GPT-4、Claude 3 Opus
- **视觉（Vision）**：支持图像处理的模型，如 GPT-4o、Claude 3.5 Sonnet
- **推理（Thinking）**：强化推理模型，如 o1 系列
- 自动从 API 获取模型列表并分类
- 支持用户手动覆盖模型类型
